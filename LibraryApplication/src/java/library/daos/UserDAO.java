...
public class UserDAO {
    ...
    public UserDTO checkLogin(String ID, String password) throws SQLException, Exception {
        ...
        try {
            ...
            if (conn != null) {
                ...
                rs = stm.executeQuery();
                if (rs.next()) {
                    result = new UserDTO(rs.getString("ID"), rs.getNString("FullName"), "***", rs.getString("RoleName"));
                }
            }
        } catch (Exception e) {
        } finally {
    ...
    public void ban(String userID, int role) throws SQLException, Exception {
        
        try {
            conn = DBUtils.getConnection();
            if (conn != null) {
                String sql = "UPDATE tblUsers SET RoleID = ?"
                        + " WHERE ID = ?";
    ...
    public void update(UserDTO dto) throws SQLException, Exception {
        try {
            ...
            if (conn != null) {
                ...
                //String sql = "SELECT userID FROM tbUser WHERE userID=?" + " AND password = ?";
                stm = conn.prepareStatement(sql);
                stm.setNString(1, dto.getFullName());
